...
class Tomograph(nn.Module):
    ...
    def forward(self, vis):
        ...
        rbm_probs = amplitude_prob, phase_prob
    ...
    def llh_loss(self, encoded_data, rbm_probs):
        ...
        unnorm_predicted_amplitude = torch.sqrt(amplitude_prob)
        predicted_phase = torch.log(phase_prob + self._eps) / 2

        amplitudes = data_amplitudes * unnorm_predicted_amplitude
        phases = data_phases + predicted_phase

        data_term = torch.sum(amplitudes * torch.cos(phases), dim=1) ** 2 \
                  + torch.sum(amplitudes * torch.sin(phases), dim=1) ** 2

        if self.gibbs:
            normalization_term = torch.mean(torch.log(amplitude_prob + self._eps))
        else:
            normalization_term = torch.log(amplitude_prob.sum() + self._eps)

        llh = torch.mean(torch.log(data_term + self._eps)) - normalization_term
    ...
    def fit(self, x, theta, n_epochs=1000, lr=1e-1, callbacks=None):
        ...
        for e in range(n_epochs):
            rbm_probs, vis = self.forward(vis)
            ...
            loss = self.llh_loss(data_states, rbm_probs)
